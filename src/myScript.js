...
function scrollMap() {
    ...
    if (map[0][0].y >= 0) {
        ...
        for (row = 0; row < map.length; row++) {
            ...
            if (num >= 95) {    //get all num >= 95 to put obstacle
    ...
    for (var row = 0; row < map.length; row++) {
        for (var col = 0; col < map[0].length; col++) {
            ...
            if (map[row][col].isObstacle === 1 &&
                map[row][col].x < player.x + SIZE/2 -20 && map[row][col].x + SIZE > player.x  &&
                map[row][col].y < player.y + SIZE/2 -20 && map[row][col].y + SIZE > player.y ) {
                // manual smaller ship  to decrease the collision chance :p
             ...
             if (map[row][col].isObstacle === 1 &&
             missile.x <= map[row][col].x + SIZE && missile.x >= map[row][col].x &&
             missile.y <= map[row][col].y + SIZE && missile.y >= map[row][col].y ){
